...
export default function FdCalculator() {
  ...
  const handleDownloadPDF = () => {
    const reportData = {
      title: 'Fixed Deposit (FD) Calculation Report',
      filename: 'fd-report.pdf',
      summary: { label: 'Estimated Maturity Amount', value: `₹${fdData.maturityAmount.toLocaleString('en-IN')}` },
      inputs: [
        { label: 'Deposit Principal', value: `₹${principal.toLocaleString('en-IN')}` },
        { label: 'Interest Rate', value: `${rate}%` },
        { label: 'Tenure', value: `${tenure} Years` },
        { label: 'Compounding Frequency', value: compounding.charAt(0).toUpperCase() + compounding.slice(1) }
      ],
      results: [
        { label: 'Invested Principal', value: `₹${principal.toLocaleString('en-IN')}` },
        { label: 'Interest Earned', value: `₹${fdData.interestEarned.toLocaleString('en-IN')}` },
        { label: 'Maturity Amount', value: `₹${fdData.maturityAmount.toLocaleString('en-IN')}` }
      ]
    };
